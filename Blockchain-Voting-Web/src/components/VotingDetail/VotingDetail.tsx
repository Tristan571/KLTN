...
export const VotingDetail: React.FC = () => {
    ...
    if(!userAddress){
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:5000/api/users/votings/users-address/${searchAddress}`
        );
        if (!response.ok) {
  ...
  const getVotingAddress = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/api/votings/address/${address}`
      );
      if (!response.ok) {
  ...
  const updateVoting = async () => {
    await fetch(`http://localhost:5000/api/votings/address/${address}`, {
      method: "PUT",
      headers: {
...
export async function getData(
  ...
  try {
    ...
    const res = await fetch(
      `/api/users/votings/users-address/${lowerCaseAddress}`
    );
